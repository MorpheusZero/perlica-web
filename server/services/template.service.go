...
func (ts *TemplateService) loadTemplates() {
	...
	if err != nil {
		fmt.Println("Failed to read base template: " + err.Error())
		return
	}

	baseTemplate, err = baseTemplate.Parse(string(baseContent))
	if err != nil {
		fmt.Println("Failed to parse base template: " + err.Error())
		return
	}
	...
	for _, file := range componentFiles {
		...
		if err != nil {
			fmt.Println("Failed to read component template " + file + ": " + err.Error())
			continue
		}

		_, err = baseTemplate.Parse(string(content))
		if err != nil {
			fmt.Println("Failed to parse component template " + file + ": " + err.Error())
			continue
		}
	}
	...
	for _, file := range pageFiles {
		content, err := templatesFS.ReadFile(file)
		if err != nil {
			fmt.Println("Failed to read page template " + file + ": " + err.Error())
			continue
		}
		...
		if err != nil {
			fmt.Println("Failed to clone base template for " + file + ": " + err.Error())
			continue
		}

		_, err = pageTemplate.Parse(string(content))
		if err != nil {
			fmt.Println("Failed to parse page template " + file + ": " + err.Error())
			continue
		}
...
func (ts *TemplateService) RenderTemplate(w http.ResponseWriter, templateName string, data interface{}) error {
	...
	if err != nil {
		fmt.Println("Failed to get template " + templateName + ": " + err.Error())
	...
	if err != nil {
		fmt.Println("Failed to execute template " + templateName + ": " + err.Error())
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		return err
	}
